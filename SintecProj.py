...
class SintecProj(object):
	...
	def get_lstm_model(self,trainX, trainY, input_dim):
		...
		xtrain_reshape = np.reshape(trainX, (trainX.shape[0], trainX.shape[1],1))
		# print(xtrain_reshape.shape)
		n_timesteps = xtrain_reshape.shape[1]
		n_features = xtrain_reshape.shape[2]
		# print(n_timesteps,n_features)

		model = Sequential(name = 'model_conv')
		model.add(Input(shape=(n_timesteps,n_features)))
		# model.add(Conv1D(15  ,kernel_size=2,strides=1, activation= 'relu', name="Conv1D_1"))
		# model.add(MaxPooling1D(pool_size=2,strides=2, name='MaxPooling1D_1'))
		# model.add(Conv1D(31  ,kernel_size=2,strides=1, activation= 'relu', name='Conv1D_2'))
		...
		model.summary()
		history = model.fit(xtrain_reshape, trainY, epochs=100, validation_split=0.2 ,verbose=1)
		return model, history
	
	def regression_LSTM(self):
		...
		for file in file_lst[0:1]:
			...
			# # ====================================================================================
			# # LSTM
			# trainX = X_train_dbp.values
			# trainY = y_train_dbp.values
			...
			# ss = trainY.std()
			# trainY = (trainY-mm)/ss
			# # trainY =  trainY / np.sqrt(np.sum(trainY**2))
	
			# # reshape input to be [samples, time steps, features]
			# trainX = np.reshape(trainX, (trainX.shape[0], 1, trainX.shape[1]))

			# model, history = self.get_lstm_model(trainX,trainY,input_dim=X_train_dbp.shape[1])
			# # make predictions
			# trainPredict = model.predict(X_train_dbp)
			# testPredict = model.predict(X_test_dbp)
