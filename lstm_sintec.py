# -*- coding: utf-8 -*-
"""Copy of lstm_sintec.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1v5DPxBz--qQ_cQ2frlROOBRjSyCiP_C6
"""
...
from keras.layers import Conv1D, Conv2D, LSTM, Dense, BatchNormalization, Dropout, Activation
...
class lstm_sintec(object):
    ...
    def data_prepare(self):
        ...
        if not os.path.exists(self.regr_path+'model'):
          os.mkdir(self.regr_path+'model')
        ...
        # Save the PCA model
        dump(pca, self.regr_path+'model/pca.joblib')
        TRAIN_PERC = 0.95
    ...
    def plot_pred(self,trainPredict,testPredict,ytrain,ytest):

        y = np.concatenate((ytrain, ytest))
        for i in range(2):
          plt.figure()
          # plt.plot(trainPredict[:,i],c='r')
          # a=len(trainPredict)
          # b=len(trainPredict)+len(testPredict)
          # x=range(a,b)
          # print(x)
          # plt.plot(x,testPredict[:,i], c='g')
          plt.plot(y[:,i],c='b')
    ...
    def main(self,units1=20,units2=20,units3=35,_learningRate=.01, SelectSBP=True):
        ...
        history = model.fit(xtrain_reshape, 
                            ...
                            verbose=0)
        model.save(self.regr_path+'model/model5.h5')
...
ls = lstm_sintec(patient = patient) #
model = load_model(ls.regr_path+'model/model5.h5')
...
for i in range(2):
  ...
  # plt.figure()
  # plt.plot(y_Sep[:,i],'*', c='b')
  # plt.plot(y_Sep_hat[:,i],'+',c='r')
  # plt.show()
  # plot Desired values and Predict values
  plt.figure()
  plt.plot(y_hat[:,i], c='r')
  plt.plot(y[:,i], c='b')
  # plt.plot(x_Sep, y_Sep[:,i], c='b')
  # plt.plot(x_Sep, y_Sep_hat[:,i], c='r')
  plt.show()
